...
In this screenshot I show on the left my administrative unit creation and below it the users in the Au. I added users in Sweden usage location to be managed by a helpdesk admin. On the right I showed a user that was not in the AU and below it one that was in the AU. On my global account, it says that the user in the Au, I couldn't change the password while another user located in USA did allow me to change password, this is because when i created the AU i set the restricted management on which prevent other admins from configuring with objects in th unit . Only the assigned helpdesk admin can control within the Au and role scoped to the Au cannot manage outside the AU.



<img width="1920" height="1080" alt="dynamic Admin Unit for sweden users  I believe require p1 license for all users to work" src="https://github.com/user-attachments/assets/9cfb87f6-5d29-4ae0-8b46-ec1e3f70b18d" />

Later i found out i could make dynamic Administrative units so i replaced the previous one with this new one. I struggled to see users fill in with the dynamic rules (user.usagelocation -eq "Sweden") then i found out all members needed p1/p2 license to work.
