...
next i went to protected actions and selected auth contexed named "protect ca changes" with all the rules. Finally i tested the protected action and it wouldnt let me change without reauthentication. **Protected actions** trigger based on a specific action someone tries to perform inside the Entra admin center or via Graph — regardless of how they got signed in.
